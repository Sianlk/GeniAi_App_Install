...
export default function AIScreen() {
  ...
  async function runAgent() {
    ...
    try {
      ...
      setLogs((p) =>
        p.map((l) =>
          l.id === id ? { ...l, status: 'done', output: r.output, confidence: r.confidence } : l,
        ),
      );
    } catch (e) {
      setLogs((p) =>
        p.map((l) => (l.id === id ? { ...l, status: 'error', output: String(e) } : l)),
      );
    } finally {
      setRunning(false);
    }
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <Text style={styles.sub}>Domain: AI platform</Text>
      <Card variant="ai" style={styles.card}>
        ...
        <ScrollView horizontal style={styles.tagRow}>
          {FEATURES.map((f) => (
            <TouchableOpacity
              key={f}
              style={[styles.tag, taskType === f && styles.tagOn]}
      ...
      {logs.map((log) => (
        <Card
          key={log.id}
          variant={log.status === 'done' ? 'default' : log.status === 'error' ? 'outlined' : 'ai'}
          style={styles.log}
        >
          ...
          {log.confidence !== undefined && (
            <Text style={styles.conf}>Confidence: {(log.confidence * 100).toFixed(0)}%</Text>
          )}
        </Card>
      ))}
    </ScrollView>
