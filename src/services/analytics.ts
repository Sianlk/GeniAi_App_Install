...
export const Analytics = {
  ...
  aiQuery: (q: string, domain: string, ms?: number) =>
    track('ai_query', { query: q.slice(0, 100), domain, ms: ms ?? 0 }),
  aiResponse: (ok: boolean, model: string, tokens?: number) =>
    track('ai_response', { ok, model, tokens: tokens ?? 0 }),
  agentTask: (type: string, ok: boolean, ms?: number) =>
    track('agent_task', { type, ok, ms: ms ?? 0 }),
  login: (method: string) => track('login', { method }),
  logout: () => track('logout'),
  featureUsed: (f: string, p?: EventProperties) => track('feature_used', { feature: f, ...p }),
  error: (code: string, msg: string, screen?: string) =>
    track('app_error', { code, msg: msg.slice(0, 200), screen: screen ?? '' }),
};
